...
* **Eficiencia de Costos y Agilidad:** Modificar el conocimiento del sistema se reduce a agregar, actualizar o borrar registros en una base de datos, lo que elimina los costos computacionales de infraestructura asociados al *fine-tuning*.
...
```mermaid
graph LR
    %% Definición del Flujo Secuencial
    DOC[Documentos Brutos<br>PDF, DOCX, HTML, JSON] --> DL[Document Loaders]
    
    DL -->|Texto Normalizado<br>+ Metadatos| TS[Text Splitters / Chunkers]
    
    TS -->|Fragmentos de Texto<br>Chunks| EM[Embedding Models]
    
    EM -->|Vectores Densos<br>ej., 1536 dim| VDB[(Vector Databases)]
    
    VDB -->|Contexto Relevante<br>Top-K Chunks| LLM[Large Language Models]
    
    PROMPT[Prompt del Usuario] --> LLM
    
    LLM --> RESP[Respuesta Natural<br>y Precisa]
...
```mermaid
graph LR
    %% Definición de Subgrafos para delimitar ciclo de vida
    subgraph "Fase de Ingesta (Offline)"
        DOC[Documento Completo] --> CHUNK{Chunking Variable<br>con Overlap}
        CHUNK -->|Bloque A| EMB[Embedding Model]
        CHUNK -->|Bloque B con muestra de A| EMB
        EMB --> VDB[(Vector Database + Metadatos)]
    end

    subgraph "Fase de Inferencia (Online)"
        QUERY[Consulta de Usuario] --> META{¿Tiene Filtros?}
        
        %% Ramificación de Metadata Filtering
        META -->|Sí: Pre-filtering| PRE[Filtrar por Tags/Fecha/ID]
        PRE --> VDB
        META -->|No| VDB
        
        %% Proceso de Recuperación y Re-ranking
        VDB -->|Búsqueda Semántica ANN| K_CHUNKS[Top-N Chunks Iniciales]
        K_CHUNKS --> RERANK[Re-ranking Model<br>Cross-Encoder]
        
        QUERY -->|Evaluación Par-a-Par| RERANK
        
        RERANK -->|Contexto Reordenado<br>Información Crítica Arriba| LLM[LLM Engine]
    end
...
7. **Generación de Prompt Enriquecido:** El orquestador toma la consulta original del usuario, concatena los textos de los fragmentos reordenados y los inserta en una plantilla de prompt estructurada (ejemplo, *"Responde a la siguiente consulta basándote exclusivamente en el contexto proporcionado..."*).
